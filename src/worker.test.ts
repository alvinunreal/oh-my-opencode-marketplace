...
function assets(): { fetch(request: Request): Promise<Response> } {
  return {
    async fetch(request) {
      ...
      if (pathname === '/v1/index.json' || pathname === '/v2/index.json') {
        return new Response('{"entries":[]}', { status: 200 });
      }
      if (pathname.startsWith('/v1/artifacts/') || pathname.startsWith('/v2/artifacts/')) {
...
describe('registry Worker routing', () => {
  ...
  test('passes both versioned roots to the shared dist asset root', async () => {
    const paths: string[] = [];
    const env = {
      ASSETS: {
